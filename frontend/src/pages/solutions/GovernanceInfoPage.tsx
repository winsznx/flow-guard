...
import { ArrowRight, Vote, Users, Shield, Eye, CheckCircle, Clock } from 'lucide-react';
  ...
  return (
    <main className="bg-background min-h-screen">
      ...
      <section className="pt-32 pb-16 px-6 lg:px-12">
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            ...
            <span className="text-sm font-medium text-accent">Treasury Governance</span>
          </motion.div>
          ...
          >
            Make treasury decisions as a group
          </motion.h1>
          ...
          >
            Proposals require approval from multiple signers before execution.
            Approval thresholds are set when treasury is created. See who approved what, and when.
          </motion.p>
      ...
      <section className="relative bg-surfaceAlt/30 overflow-hidden">
        <div className="py-12 md:py-20">
          {[
            ...
            { icon: Clock, title: 'Time-based execution', description: 'Proposals can be queued with timelocks.', detail: 'Gives everyone time to review before execution.' },
            { icon: Vote, title: 'Optional token voting', description: 'For larger groups, use governance tokens instead of fixed signers.', detail: 'Vote weight based on token holdings.' },
          ].map((item, index) => {
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="space-y-6">
            {[
              {
                step: '1',
                title: 'Someone proposes a spending request',
                description: 'Any signer can create a proposal to spend treasury funds. Includes recipient, amount, and reason.',
              },
              {
                step: '2',
                title: 'Other signers review and approve',
                description: "Each signer reviews the proposal. They approve or reject based on the treasury's rules.",
              },
              {
                step: '3',
                title: 'Proposal executes when threshold is met',
                description: 'Once enough approvals are collected (e.g., 3 of 5), the proposal becomes executable.',
              },
              {
                step: '4',
                title: 'Funds are released',
                description: 'Anyone can trigger the execution after approvals are met. Funds go to the approved recipient.',
              },
            ].map((step, index) => (
              <motion.div
                key={index}
                ...
                className="flex gap-6 p-6 bg-surfaceAlt/50 rounded-2xl"
              >
      ...
      <section className="py-20 px-6 lg:px-12 bg-surfaceAlt/30">
        <div className="max-w-5xl mx-auto">
          <h2 className="font-display text-4xl text-center mb-12 text-textPrimary">Common governance setups</h2>
          <div className="space-y-6">
            {[
              {
              ...
              },
              {
                ...
                description: 'Three partners. Two must approve, but execution is delayed 48 hours for review.',
              },
              {
                title: 'Token-weighted voting',
                description: 'Community treasury. Governance token holders vote on proposals. Majority wins.',
              },
            ].map((useCase, index) => (
              <motion.div
                key={index}
                ...
                viewport={{ once: true }}
                transition={{ delay: index * 0.1 }}
                className="p-6 border-l-4 border-accent bg-surface rounded-r-xl"
              >
      ...
      <section className="py-24 px-6 lg:px-12 bg-accent/5">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="font-display text-4xl md:text-5xl mb-6 text-textPrimary">
            Ready to set up treasury governance?
          </h2>
          <p className="text-xl text-textSecondary mb-10">
            Create a treasury with approval rules that work for your team
          </p>
