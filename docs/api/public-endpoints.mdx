...
| `executor` | `Workers` | Scheduled transaction executor, sanitized from `EXECUTOR_STATUS_URL`. |
...
Remote worker payloads (indexer, executor) are sanitized before re-emission. Only the upstream `service.status` field is mapped through `normalizeStatus`. Hostnames, queue depths, signing-key flags, and other internals are dropped.
