...
- A future revision can reintroduce early reclaim safely by binding the lock to a specific `ProposalCovenant` instance via a `proposalLockingBytecodeHash` constructor parameter and requiring an input from that proposal in `CANCELLED` or `EXPIRED` status.
