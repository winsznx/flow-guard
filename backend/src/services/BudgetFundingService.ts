/**
 * Budget Funding Service
 * Builds funding transactions for budget plan contracts
 */
...
export interface BudgetFundingParams {
  ...
  totalAmount: number | string | bigint; // satoshis or token amount
...
export class BudgetFundingService {
  ...
  /**
   * Build a funding transaction for a budget plan contract
   *
   ...
   */
  async buildFundingTransaction(params: BudgetFundingParams): Promise<UnsignedFundingTransaction> {
    const {
      contractAddress,
      senderAddress,
      totalAmount,
    ...
    if (tokenType === 'FUNGIBLE_TOKEN') {
      ...
      if (!categoryAnchor) {
        throw new Error(
          'Cannot mint budget state NFT: sender wallet needs a spendable BCH UTXO with outpoint index 0',
        );
      }
