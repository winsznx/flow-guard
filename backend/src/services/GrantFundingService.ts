...
export interface GrantFundingTransactionParams {
...
export class GrantFundingService {
  ...
  async buildFundingTransaction(params: GrantFundingTransactionParams): Promise<UnsignedFundingTransaction> {
    const {
      contractAddress,
      creatorAddress,
      totalAmount,
    ...
    const contractOutput = getRequiredContractFundingSatoshis('airdrop', tokenType, totalAmountOnChain);
    ...
    if (tokenType === 'FUNGIBLE_TOKEN') {
      ...
      const selection = selectTokenFundingInputs(utxos, tokenCategory, totalAmountOnChain, 'airdrop');
      ...
      if (!categoryAnchor) {
        throw new Error(
          'Cannot mint grant state NFT: creator wallet needs a spendable BCH UTXO with outpoint index 0',
        );
      }
    ...
    if (!stateTokenCategory) {
      throw new Error('Missing state token category for grant funding output');
    }
    ...
    const wcTransaction = buildFundingWcTransaction({
      ...
      outputs,
      userPrompt: `Fund grant contract ${contractAddress}`,
      broadcast: false,
    });
