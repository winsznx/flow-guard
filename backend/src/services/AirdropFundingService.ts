/**
 * Airdrop Funding Service
 * Builds funding transactions for airdrop contracts
 */
...
export interface FundingTransactionParams {
  ...
  totalAmount: number | string | bigint; // Total tokens/BCH for airdrop
  merkleRoot?: string; // optional: public campaigns may not use merkle proofs
...
export class AirdropFundingService {
  ...
  async buildFundingTransaction(params: FundingTransactionParams): Promise<UnsignedFundingTransaction> {
    ...
    if (tokenType === 'FUNGIBLE_TOKEN') {
      ...
      if (!categoryAnchor) {
        throw new Error(
          'Cannot mint airdrop state NFT: creator wallet needs a spendable BCH UTXO with outpoint index 0',
        );
      }
