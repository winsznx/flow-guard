/**
 * Grant Control Service
 * Handles pause, cancel, and transfer operations for GrantCovenant contracts
 */
...
export interface GrantControlBuildParams {
...
export interface GrantTransferBuildParams extends GrantControlBuildParams {
  newRecipientAddress: string;
}

export interface GrantControlBuildResult {
...
export interface GrantTransferBuildResult {
  wcTransaction: WcTransactionObject;
  newRecipientHash: string;
}

export class GrantControlService {
  ...
  async buildPauseTransaction(params: GrantControlBuildParams): Promise<GrantControlBuildResult> {
    ...
    if (status !== 0) {
      throw new Error('Grant must be ACTIVE to pause');
    }
    if ((flags & 0x01) !== 0x01) {
      throw new Error('Grant is not configured as cancelable/pausable');
    }
    ...
    if (stateOutputSatoshis < 546n) {
      throw new Error('Insufficient contract balance to pause grant');
    }
    ...
    const wcTransaction = this.forceFinalSequences(txBuilder.generateWcTransactionObject({
      broadcast: true,
      userPrompt: 'Pause grant program',
    }));
  ...
  async buildCancelTransaction(params: GrantControlBuildParams): Promise<GrantControlBuildResult> {
    const artifact = ContractFactory.getArtifact('GrantCovenant');
    const contract = new Contract(artifact, params.constructorParams, { provider: this.provider });
    const { contractUtxo, commitment } = await this.getContractState(params.contractAddress, params.currentCommitment);
    ...
    if (status !== 0 && status !== 1) {
      throw new Error('Grant must be ACTIVE or PAUSED to cancel');
    }
    if ((flags & 0x01) !== 0x01) {
    ...
    if (params.tokenType === 'FUNGIBLE_TOKEN') {
      ...
      if (remainingAmount < 546n) {
        throw new Error('Remaining BCH amount is below dust and cannot be cancelled');
      }
      if (contractUtxo.satoshis < remainingAmount) {
        throw new Error('Contract balance is below remaining BCH amount');
      }
      txBuilder.addOutput({
        to: cancelReturnAddress,
        amount: remainingAmount,
      });
      spentSatoshis += remainingAmount;
    }
    ...
    const wcTransaction = this.forceFinalSequences(txBuilder.generateWcTransactionObject({
      broadcast: true,
      userPrompt: 'Cancel grant program and recover remaining funds',
    }));
  ...
  async buildTransferTransaction(params: GrantTransferBuildParams): Promise<GrantTransferBuildResult> {
    const artifact = ContractFactory.getArtifact('GrantCovenant');
    const contract = new Contract(artifact, params.constructorParams, { provider: this.provider });
    const { contractUtxo, commitment } = await this.getContractState(params.contractAddress, params.currentCommitment);
    ...
    const flags = commitment[1] ?? 0;
    if (status !== 0) {
      throw new Error('Grant must be ACTIVE to transfer');
    }
    if ((flags & 0x02) !== 0x02) {
      throw new Error('Grant is not configured as transferable');
    }

    const newRecipientHash = this.addressToHash160(params.newRecipientAddress);
    const newCommitment = new Uint8Array(40);
    newCommitment.set(commitment.slice(0, 16), 0);
    ...
    const stateOutputSatoshis = feePayer
      ...
      : contractUtxo.satoshis - feeReserve;
    if (stateOutputSatoshis < 546n) {
      throw new Error('Insufficient contract balance for transfer');
    }
    ...
    const wcTransaction = this.forceFinalSequences(txBuilder.generateWcTransactionObject({
      broadcast: true,
      userPrompt: 'Transfer grant to new recipient',
    }));

    return {
      wcTransaction,
      newRecipientHash: binToHex(newRecipientHash),
    };
  }
  ...
  private addressToHash160(address: string): Uint8Array {
    ...
    const bytecode = decoded.bytecode;
    if (bytecode.length !== 25 || bytecode[0] !== 0x76 || bytecode[1] !== 0xa9 || bytecode[2] !== 0x14) {
      throw new Error('Only P2PKH addresses are supported for grant recipients');
    }
  ...
  private async getContractState(contractAddress: string, fallbackCommitment: string): Promise<{
    ...
    if (!contractUtxo.token?.nft) {
      throw new Error('Grant contract UTXO is missing required state NFT');
    }
    ...
    if (commitment.length < 36) {
      throw new Error('Invalid grant state commitment');
    }
  ...
  private p2pkhFromHash(hash20: Uint8Array): string {
    ...
    if (typeof encoded === 'string') {
      throw new Error(`Failed to encode P2PKH address: ${encoded}`);
    }
  ...
  private async selectFeePayerInputs(address: string, requiredFee: bigint): Promise<{
    ...
    if (total < requiredFee) {
      throw new Error(
        `Signer ${address} needs at least ${requiredFee} sats of BCH UTXOs to pay network fee`
      );
    }
