/**
 * Reward Funding Service
 * Builds funding transactions for reward contracts
 */
...
export interface RewardFundingTransactionParams {
...
export class RewardFundingService {
  ...
  async buildFundingTransaction(params: RewardFundingTransactionParams): Promise<UnsignedFundingTransaction> {
    const {
      contractAddress,
      creatorAddress,
      totalPool,
    ...
    if (tokenType === 'FUNGIBLE_TOKEN') {
      ...
      if (!categoryAnchor) {
        throw new Error(
          'Cannot mint reward state NFT: creator wallet needs a spendable BCH UTXO with outpoint index 0',
        );
      }
    ...
    if (!stateTokenCategory) {
      throw new Error('Missing state token category for reward funding output');
    }
    ...
    const wcTransaction = buildFundingWcTransaction({
      ...
      outputs,
      userPrompt: `Fund reward contract ${contractAddress}`,
      broadcast: false,
    });
